...
$(document).ready(function () {
  ...
  $form.on('submit', function(event) {
      ...
      // default event
      event.preventDefault() var form_data = $(this)
          .serializeArray() addNewRow(form_data)
  })
  ...
  function addNewRow(data) {
      ...
      for (var i = 0; i < data.length; i++) {
              ...
              $newTd.text(data[i].value)
                  // where we put the form input value
              $newTr.append($newTd)
          }
          // create new button
      var $newButton = $(
              '<td><button class="btn btn-xs btn-danger removeHome">Remove</button></td>'
          ) $newTr.append($newButton) $tbody.append($newTr)
